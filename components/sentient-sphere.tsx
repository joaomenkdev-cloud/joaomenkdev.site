...
function Sphere() {
  ...
  const fragmentShader = `
    ...
    void main() {
      ...
      // Gold color: #c9955f -> rgb(201, 149, 95) -> normalized
      vec3 goldColor = vec3(0.788, 0.584, 0.373);
      vec3 creamColor = vec3(0.859, 0.831, 0.773);
      vec3 color = mix(goldColor, creamColor, intensity);
      
      float line = smoothstep(0.0, 0.02, abs(fract(vUv.x * 20.0) - 0.5));
      line *= smoothstep(0.0, 0.02, abs(fract(vUv.y * 20.0) - 0.5));
      
      gl_FragColor = vec4(color * (1.0 - line * 0.5), 0.5);
    }
  `
  ...
  if (!mounted) {
    return (
      <div className="w-full h-full flex items-center justify-center">
        <div className="w-64 h-64 rounded-full border border-[rgba(201,149,95,0.2)] animate-pulse" />
      </div>
    )
  }
